...
- **reads the area's existing epic(s) first — plus any `CONTEXT.md` / ADR log** (the settled-decision record, e.g. the one `domain-modeling` writes), **plus the rejection memory the delta read-path supplies** ([SETUP.md](SETUP.md) §Delta read-path — seams already killed and still unchanged, so they aren't re-proposed and re-killed) — to build the exclusion list,
...
- **rejected[]** = SHA-anchored negative-result memory — but **only the oracle-anchored rejections**: those whose evidence *replayed* (a single-adapter hypothetical, or a deletion-test / exclusion rejection that reproduced its call sites) and so carry a real `oracle_at_base`. A candidate that died *at* the oracle in step 1 (a fabricated or cherry-picked count that couldn't reproduce) gets **no** entry — there's nothing to re-test — so it counts toward `proposed` and the PRD's prose rejection list but not here. Hence `len(rejected[]) ≤ rejected count`; the scalar identity above uses the PRD's full count, never the array length. Each entry names the rejected `seam`, its `paths`, the `oracle_at_base` (the replayable fact at `base`, e.g. "1 call site"), and a **pointer** to the PRD for the prose reason — the seam + base + paths are exactly what a later sweep needs to decide whether the rejection has expired, *without* the receipt ever owning the reason. All of a receipt's rejections share its **top-level `base`** — there is no per-entry base, since each receipt holds only its own generation's kills — and the read-path ([SETUP.md](SETUP.md) §Delta read-path) anchors the revival test to that base + the seam's `paths`. A single-adapter hypothetical rightly rejected at `base` becomes a real seam once a *second adapter* lands — often in a **sibling** directory — so record an adapter-class rejection's `paths` as the seam's **domain / parent scope** (`src/pay/`, not the first adapter's `src/pay/stripe/`); otherwise the revival test, scoped to the narrow dir, would miss the new adapter and keep wrongly excluding the seam just as it earns its keep. Keep `paths` plain directory/file pathspecs (no globs — git matches `*` literally).
...
- A **no-op-skipped** area (the delta read-path skipped it because nothing changed — [SETUP.md](SETUP.md) §Delta read-path) writes a *minimal* receipt **into a fresh `…-deepening-delta<N+1>/` dir** (the same suffix-bump a swept delta would use, but `RECEIPT.md` only — no PRD/issues): bumped `generation`, `base` = the current preflight base, `proposed: 0` (so `survival_rate: n/a`, `noisy_proposer: false`), `churn_30d` still computed, empty `filed`/`rejected`, and a `noop: no change since <last_base>` line recording that it was confirmed clean by the no-op test rather than a full sweep. `git add` that dir, commit, tick — a **receipt-only `.scratch/` dir is a valid committed area**, so `git log -- .scratch/`, resume, and the next delta's no-op test all advance past it.

> **Read-path (wired — [SETUP.md](SETUP.md) §Delta read-path).** A later delta sweep consumes these receipts after forking the worktree, before proposers fan out: the newest generation's `base`/`paths` drive an **area-level no-op test** (area unchanged since `base` → skipped, no proposer runs); and `rejected[]` **unioned across all generations** (each receipt holds only its own generation's rejections, so newest-only would forget older kills) drives a **per-seam** no-op test — unchanged → excluded so a killed seam isn't re-litigated, changed → re-evaluated fresh. Both decide on one shared `unchanged()` predicate that **fails open**: any uncertainty (unreachable base, **not an ancestor of the tip**, moved/empty paths, nonzero `git` exit, missing/pre-v0.4.0 receipt) re-sweeps rather than silently suppress. The receipt stays a hint — never authority, never read on resume.
...
…where **Proposed / Survival** expose the multiple-comparisons signal — a low survival rate over many proposals is a noisy proposer whose survivors had to clear the raised bar (step 5), tagged `noisy_proposer` in Notes — and **Durability** is `durable` / `active` / `collision-risk` from `churn_30d` (a collision-risk Strong is a short-half-life finding: implement it first or hold). A **no-op-skipped** area (delta read-path, [SETUP.md](SETUP.md)) is a `Proposed 0` row tagged *"skipped — no change @ `<base>`"*. Plus the branch, the worktree path, the `.scratch/` paths, any `[!] needs manual look` areas with why, the count of rejected seams the read-path **excluded** vs **re-evaluated** (and any receipts it couldn't use → fell back to a full sweep), and whether the base went stale mid-sweep. Then ask whether the user wants the optional implement phase ([EXTENDING.md](EXTENDING.md)).
