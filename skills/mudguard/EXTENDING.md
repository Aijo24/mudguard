...
- Write the implement per-iteration spec + area checklist (for the ralph driver, `.ralph/PROMPT.md` / `.ralph/fix_plan.md` — see [DRIVERS.md](DRIVERS.md)): **one ticket per iteration**, **Strong-first** and **durability-aware** (the sweep's receipt records each area's `churn_30d` — a `collision-risk` area is a merge-conflict magnet, so grab its tickets *early* before the area drifts out from under them, or skip them this round; a `durable` finding in a stable core can wait), behaviour-preserving (deletion test), each a vertical slice (the deep module + every call site repointed + tests at the new interface + the old inline copies deleted), then **your gates**, a changelog entry, and a conventional commit staging only changed files (never `git add -A`).
