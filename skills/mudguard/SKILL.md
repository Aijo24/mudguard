...
- **Delta-aware.** If `.scratch/*-deepening*` epics already exist for an area, exclude already-filed/shipped seams and write new candidates to `…-delta` (bump the suffix). "Zero new candidates" is a valid, honest result. Each area leaves a SHA-anchored `RECEIPT.md` (see [SWEEP.md](SWEEP.md)) recording the base it was verified against, so a later sweep can anchor its exclusions on a commit, not on prose.
